...
def extrair_e_tratar_indicadores_educacionais():
    ...
    print(f"Executando a extração dos dados (filtrando anos de 2021 a 2026)...")
    try:
        ...
        if df.empty:
            ...
            print("Pode ser que a base do INEP na Base dos Dados ainda não tenha sido atualizada com esses anos mais recentes.")
            return df
            
        print(f"✓ {len(df)} registros extraídos. Iniciando tratamento e limpeza...")
        ...
        colunas_metricas = [col for col in df.columns if col not in ['ano', 'localizacao', 'rede']]
        
        df[colunas_metricas] = df[colunas_metricas].astype(float).round(1)
        
        os.makedirs('data', exist_ok=True)
        caminho_arquivo = os.path.join('data', 'indicadores_educacionais_brasil_2021_2026.csv')
