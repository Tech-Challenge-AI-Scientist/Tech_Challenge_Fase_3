"""Função de extração e tratamento de dados dos Indicadores Educacionais por UF."""

import os
from google.cloud import bigquery


def extrair_e_tratar_indicadores_uf():
    """Função para extrair e tratar dados dos Indicadores Educacionais por UF para os anos de 2021 a 2026."""
    ...
    print("Executando a extração dos dados estaduais (2021 a 2026)...")
    try:
        ...
        print(f"✓ {len(df)} registros extraídos. Iniciando tratamento...")

        # 3. O mesmo mapeamento inteligente de prefixos
        mapeamento_prefixos = {
        ...
        colunas_dimensoes = ["ano", "sigla_uf", "localizacao", "rede"]
        colunas_metricas = [
            col for col in df.columns if col not in colunas_dimensoes
        ]
        ...
        caminho_arquivo = os.path.join(
            "data", "indicadores_educacionais_uf_2021_2026.csv"
        )
        ...
        print(df.head())
        print(f"\nSucesso! Arquivo estadual salvo em: {caminho_arquivo}")
...
df_educacao_uf = extrair_e_tratar_indicadores_uf()
