...
def extrair_e_tratar_indicadores_uf():
    ...
    print(f"Executando a extração dos dados estaduais (2021 a 2026)...")
    try:
        df = client.query(query).to_dataframe()
        
        if df.empty:
            print("Aviso: A consulta não retornou nenhum dado para o período de 2021 a 2026.")
            return df
        ...
        colunas_metricas = [col for col in df.columns if col not in colunas_dimensoes]
        
        df[colunas_metricas] = df[colunas_metricas].astype(float).round(1)
        

        os.makedirs('data', exist_ok=True)
        caminho_arquivo = os.path.join('data', 'indicadores_educacionais_uf_2021_2026.csv')
