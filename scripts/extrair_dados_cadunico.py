"""Função de extração de dados do CadÚnico.

O script baixa os dados de cada ano, consolida em um único DataFrame.
Trata os dados e salva em um arquivo CSV.
"""
...
def extrair_cadunico_multiplos_anos():
    """Função para extrair dados do CadÚnico para os anos de 2023 a 2026."""
    ...
    for ano in anos:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Erro na requisição do ano {ano}: Status {response.status_code}"
            )
    ...
    if lista_dfs:
        ...
        df_completo["ano_mes"] = pd.to_datetime(
            df_completo["ano_mes"].astype(str), format="%Y%m"
        ).dt.strftime("%Y-%m")

        colunas_qtd = [
            col for col in df_completo.columns if col.startswith("qtd_")
        ]
        df_completo[colunas_qtd] = (
            df_completo[colunas_qtd].fillna(0).astype(int)
        )

        colunas_taxa = [
            col for col in df_completo.columns if col.startswith("taxa_")
        ]
        df_completo[colunas_taxa] = (
            df_completo[colunas_taxa].astype(float).round(2)
        )
        ...
        df_completo.to_csv(
            f"./data/{nome_arquivo}", index=False, encoding="utf-8"
        )
